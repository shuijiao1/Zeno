...
- 服务器元数据：到期日、账单周期、显示顺序、国家码/国旗、公网 IPv4、公网 IPv6。
- 状态规则支持按服务器范围生效。
...
1. 自动 GeoIP / 公网 IPv4 / 公网 IPv6 识别：优先简单可替换实现，失败不影响 Agent 心跳。
2. 多节点铺 Agent / 从现有服务器清单同步节点：在发布工具稳定后推进。
3. 安装文档打磨和开源前自部署说明。
4. 后续服务监控状态页 / 历史页：暂缓，确认需要后再做。
5. UI polish：图片上传、拖拽排序、更多数据维护增强等，放在主线闭环之后。
