...
4. Admin 管理动作：服务器创建/编辑/安装命令、游客隐藏、目标创建/编辑/删除/排序/分配、通知渠道/类型/测试发送/发送记录、状态规则范围、维护清理。
...
服务器和探测目标都支持 `hide_for_guest`：

- 对服务器：后台和 Agent 上报不受影响，但 Public summary、节点详情资源历史和节点详情延迟接口都不可见。
- 对探测目标：Agent 仍继续执行并写入历史，后台仍可管理；Public summary latency、首页默认图和节点延迟详情会过滤隐藏目标。
- 这是展示层隐私/整理能力，不是停用监控能力；不要用它替代 `disabled` 或目标 `enabled=false`。
