...
- Web UI：主页服务器卡片、服务详情页、节点详情页、延迟图、资源历史图、移动端紧凑布局；首页不再单独展示监控服务列表。
- Admin API/UI：单管理员登录、改密码、退出登录、登录失败限速、服务器、服务器排序、Agent 安装命令轮换/复制、Agent 接入 URL、探针目标、探针目标排序、节点分配、通知、当前异常、外观设置；外观和 Agent 接入 URL 保存前会先校验格式。
