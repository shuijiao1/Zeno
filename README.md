...
- Public API：主页 summary、监控服务状态、服务多节点历史、节点延迟、节点状态历史、公开外观设置；外观图片只配置 URL / 站内静态路径。
- Web UI：主页卡片、监控服务状态、服务详情页、节点详情页、延迟图、资源历史图、移动端紧凑布局。
- Admin API/UI：服务器、服务器排序、Agent 安装命令轮换/复制、Agent 接入 URL、探针目标、探针目标排序、节点分配、通知、当前异常、外观设置；外观和 Agent 接入 URL 保存前会先校验格式。
