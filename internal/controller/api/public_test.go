...
func TestSummaryWebSocketPublishesAgentHeartbeatAndHostUpdates(t *testing.T) {
	store, err := OpenSQLiteStore(filepath.Join(t.TempDir(), "zeno.db"))
	if err != nil {
		t.Fatalf("open sqlite store: %v", err)
	}
	...
	if !strings.Contains(string(initial), `"status":"offline"`) {
		t.Fatalf("initial websocket summary = %s, want offline", string(initial))
	}

	heartbeat := []byte(`{"ts":` + strconv.FormatInt(time.Now().UTC().Unix(), 10) + `,"status":"online","agent_version":"agent-test"}`)
	heartbeatRequest, err := http.NewRequest(http.MethodPost, server.URL+"/api/agent/v1/heartbeat", bytes.NewReader(heartbeat))
	if err != nil {
	...
	heartbeatResponse, err := server.Client().Do(heartbeatRequest)
	if err != nil {
	...
	_, heartbeatUpdate, err := conn.ReadMessage()
	if err != nil {
		t.Fatalf("read heartbeat update: %v", err)
	}
	if !strings.Contains(string(heartbeatUpdate), `"status":"online"`) || !strings.Contains(string(heartbeatUpdate), `"os":"debian"`) {
		t.Fatalf("heartbeat update = %s, want online summary", string(heartbeatUpdate))
	}
	...
	host := map[string]any{
		...
		"os_version":         "25H2",
		"kernel":             "build 26200.8655",
		"arch":               "x86_64",
		"virtualization":     "hyper-v",
		"cpu_model":          "Intel Core Ultra",
		"cpu_cores":          16,
		"memory_total_bytes": int64(32 * 1024 * 1024 * 1024),
		"disk_total_bytes":   int64(1024 * 1024 * 1024 * 1024),
		"boot_time":          time.Now().UTC().Add(-4 * time.Hour).Unix(),
	...
	hostRequest, err := http.NewRequest(http.MethodPost, server.URL+"/api/agent/v1/host", bytes.NewReader(hostPayload))
	if err != nil {
		t.Fatalf("new host request: %v", err)
	}
	hostRequest.Header.Set("X-Node-ID", "hytron")
	hostRequest.Header.Set("Authorization", "Bearer test-agent-token")
	hostRequest.Header.Set("Content-Type", "application/json")
	hostResponse, err := server.Client().Do(hostRequest)
	if err != nil {
		t.Fatalf("post host: %v", err)
	}
	defer hostResponse.Body.Close()
	if hostResponse.StatusCode != http.StatusAccepted {
		t.Fatalf("host status = %d, want 202; body=%s", hostResponse.StatusCode, readAllString(t, hostResponse.Body))
	}

	_, hostUpdate, err := conn.ReadMessage()
	if err != nil {
		t.Fatalf("read host update: %v", err)
	}
	if !strings.Contains(string(hostUpdate), `"os":"Windows 10 Pro"`) || !strings.Contains(string(hostUpdate), `"cpu_model":"Intel Core Ultra"`) || !strings.Contains(string(hostUpdate), `"memory_total_bytes":34359738368`) {
		t.Fatalf("host update = %s, want live host facts", string(hostUpdate))
	}
}
...
func TestSummaryWebSocketInitialFrameCanUseCachedPayload(t *testing.T) {
	...
	handler := &handler{store: store, liveHub: newLiveUpdateHub()}
	...
	defer server.Close()
	conn, response, err := websocket.DefaultDialer.Dial("ws"+strings.TrimPrefix(server.URL, "http")+"/api/public/v1/summary/ws", nil)
	if err != nil {
		t.Fatalf("open summary websocket: %v", err)
	}
	if response != nil && response.Body != nil {
