...
describe('homepage and admin shell layout', () => {
  ...
  it('uses a compact homepage summary without duplicate title/online-rate tiles', () => {
    ...
    expect(styles).toContain('grid-template-columns: minmax(188px, .52fr) minmax(540px, 1.48fr)')
  ...
  it('polishes backend mobile navigation, modal actions, and save buttons without overlap', () => {
    ...
    expect(styles).toContain('.admin-block-heading .admin-primary-action { height: 36px; padding: 0 12px; font-size: 13px; }')
...
describe('Kulin-inspired color polish', () => {
  ...
  it('uses solid accent colors without gradients and keeps key identity emphasized', () => {
    ...
    expect(styles).toContain('.node-metric strong { min-width: 0; margin-left: auto; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; color: var(--metric-accent); font-weight: 500')
