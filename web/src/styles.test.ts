...
describe('homepage and admin shell layout', () => {
  ...
  it('uses a compact homepage summary without duplicate title/online-rate tiles', () => {
    ...
    expect(styles).toContain('.home-top-card::before')
    expect(styles).toContain('grid-template-columns: minmax(200px, .72fr) minmax(430px, 1.28fr)')
...
describe('Kulin-inspired color polish', () => {
  ...
  it('uses grey labels with black values and emphasizes latency plus server identity', () => {
    ...
    expect(styles).toContain('.home-summary__status-line strong')
    expect(styles).toContain('font-size: 27px')
